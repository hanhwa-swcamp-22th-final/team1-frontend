...
const ROLE_TABS = [
  ...
  { key: 'WH_WORKER',  label: 'WH_WORKER (창고 작업자)',  count: 6  },
]
      ...
      <table class="matrix-table">
        ...
        <tbody>
          <template v-for="section in SECTIONS" :key="section.label">
            ...
            <!-- 데이터 행 -->
            <tr v-for="item in section.items" :key="item.id" class="data-row">
              ...
              <td><span class="req-link">{{ item.req }}</span></td>
              <td class="col-perm">
                <button
                  class="perm-check"
                  :class="getPerm(item.id, activeRole, 'r') ? 'perm-on' : 'perm-off'"
                  @click="togglePerm(item.id, activeRole, 'r')"
                >
                  <svg v-if="getPerm(item.id, activeRole, 'r')" width="12" height="12" viewBox="0 0 12 12" fill="none">
              ...
              <td class="col-perm">
                <button
                  class="perm-check"
                  :class="getPerm(item.id, activeRole, 'w') ? 'perm-on' : 'perm-off'"
                  @click="togglePerm(item.id, activeRole, 'w')"
                >
                  <svg v-if="getPerm(item.id, activeRole, 'w')" width="12" height="12" viewBox="0 0 12 12" fill="none">
                    <path d="M2 6l3 3 5-5" stroke="var(--green)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                </button>
              </td>
              <td class="col-perm">
                <button
                  class="perm-check"
                  :class="getPerm(item.id, activeRole, 'd') ? 'perm-on' : 'perm-off'"
                  @click="togglePerm(item.id, activeRole, 'd')"
                >
                  <svg v-if="getPerm(item.id, activeRole, 'd')" width="12" height="12" viewBox="0 0 12 12" fill="none">
                    <path d="M2 6l3 3 5-5" stroke="var(--green)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                </button>
              </td>
              <td><span class="api-key">{{ item.apiKey }}</span></td>
            </tr>
          </template>
        </tbody>
      </table>
