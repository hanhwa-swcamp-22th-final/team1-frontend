...
## 2026-04-14 주문 식별자 필드명 정리

### Decision
- 주문 API와 프론트 내부 데이터 키는 `orderId`로 통일하고, 화면 라벨은 계속 `주문번호`로 유지한다.

### Context
- 백엔드 command 쪽 주문 생성과 상세/취소 API는 이미 `orderId`를 기준 식별자로 사용하고 있다.
- 반면 일부 query 목록 DTO는 `sales_order.order_id`를 `orderNo`로 alias해서 내려주고 있었고, 프론트도 `orderId`와 `orderNo`를 혼용하고 있었다.
- 이 혼용 때문에 등록/목록/상세/취소를 연결할 때 같은 값을 두 이름으로 다뤄야 했고, 화면별 정규화 코드가 불필요하게 복잡해졌다.

### Alternatives Considered
- 백엔드와 프론트 모두 `orderNo`를 유지하고 `orderId`는 path variable 용도로만 제한하는 방식
- `orderId`와 `orderNo`를 별도 필드로 분리해 동시에 운영하는 방식

### Why
- 현재 시스템에서 실제로 서버가 생성하고 보장하는 식별자는 `orderId` 하나이므로, 코드 필드명도 여기에 맞추는 편이 가장 일관된다.
- 화면에서는 사용자에게 `주문번호`라는 라벨을 유지해도 되므로 UX를 바꾸지 않고 내부 계약만 정리할 수 있다.
- 목록 DTO alias와 프론트 호환 코드를 `orderId` 기준으로 맞추면 이후 셀러/관리자/창고관리자 화면 정리가 같은 방향으로 이어진다.
