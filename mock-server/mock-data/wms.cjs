// mock-data/wms.cjs — WMS(창고관리) 관련 mock 데이터
// 사용처: routes/asn.cjs, routes/wms.cjs
...
const WMS_STATUS = [
  {
    id: 1, name: 'LA West Coast Hub', tag: '메인 거점', progress: 92,
    status: 'active', statusLabel: '정상 운영중',
    kpis: [
  ...
  {
    id: 2, name: 'Central Dallas Center', tag: null, progress: 65,
    status: 'active', statusLabel: '정상 운영중',
    kpis: [
  ...
  {
    id: 3, name: 'East NY Hub', tag: null, progress: 48,
    status: 'idle', statusLabel: '주의 모니터링',
    kpis: [
...
const WMS_SUMMARY = {
  totalCount: 4, activeCount: 3, totalInventory: 18420, todayOutbound: 203, avgLocationUtil: 69,
}

const WMS_LIST = [
  {
    id: 1, code: 'WH-US-LA-001', name: 'LA West Coast Hub', location: 'Los Angeles, CA',
    status: 'ACTIVE',
    stats: { inventory: 8240, todayOutbound: 89, pendingAsn: 3, sellerCount: 6 },
    locationUtil: 72,
    manager: { name: '김태영', email: 't.kim@glsm-la.com', phone: '+1-213-555-0182', lastLogin: '오늘 14:22', status: 'ACTIVE' },
  },
  {
    id: 2, code: 'WH-US-TX-002', name: 'Central Dallas Center', location: 'Dallas, TX',
    status: 'ACTIVE',
    stats: { inventory: 5180, todayOutbound: 54, pendingAsn: 1, sellerCount: 4 },
    locationUtil: 48,
    manager: { name: '박지훈', email: 'j.park@glsm-tx.com', phone: '+1-214-555-0391', lastLogin: '2시간 전', status: 'ACTIVE' },
  },
  {
    id: 3, code: 'WH-US-NY-003', name: 'East NY Hub', location: 'Newark, NJ',
    status: 'CAUTION',
    stats: { inventory: 5000, todayOutbound: 60, pendingAsn: 2, sellerCount: 5 },
    locationUtil: 88,
    manager: { name: '이소윤', email: 's.lee@glsm-ny.com', phone: '+1-201-555-0844', lastLogin: '오늘 09:11', status: 'CAUTION' },
  },
  {
    id: 4, code: 'WH-US-IL-004', name: 'Chicago Midwest Depot', location: 'Chicago, IL',
    status: 'INACTIVE',
...
const WAREHOUSE_LOCATIONS = {
  ...
  1: [
    { zone: 'A', label: 'Zone A — 일반보관 구역', utilPct: 60, available: 12, total: 30,
      racks: [
        { name: 'RACK A-01', bins: makeBins('A', 1, 4) }, { name: 'RACK A-02', bins: makeBins('A', 2, 3) },
        { name: 'RACK A-03', bins: makeBins('A', 3, 4) }, { name: 'RACK A-04', bins: makeBins('A', 4, 3) },
        { name: 'RACK A-05', bins: makeBins('A', 5, 4) },
      ] },
    { zone: 'B', label: 'Zone B — 냉장보관 구역', utilPct: 75, available: 7, total: 30,
      racks: [
        { name: 'RACK B-01', bins: makeBins('B', 1, 5) }, { name: 'RACK B-02', bins: makeBins('B', 2, 4) },
        { name: 'RACK B-03', bins: makeBins('B', 3, 5) }, { name: 'RACK B-04', bins: makeBins('B', 4, 4) },
        { name: 'RACK B-05', bins: makeBins('B', 5, 5) },
      ] },
    { zone: 'C', label: 'Zone C — 대형물류 구역', utilPct: 80, available: 5, total: 25,
      racks: [
        { name: 'RACK C-01', bins: makeBins('C', 1, 5) }, { name: 'RACK C-02', bins: makeBins('C', 2, 5) },
        { name: 'RACK C-03', bins: makeBins('C', 3, 4) }, { name: 'RACK C-04', bins: makeBins('C', 4, 5) },
  ...
  2: [
    { zone: 'A', label: 'Zone A — 일반보관 구역', utilPct: 45, available: 16, total: 30,
      racks: [
        { name: 'RACK A-01', bins: makeBins('A', 1, 2) }, { name: 'RACK A-02', bins: makeBins('A', 2, 3) },
        { name: 'RACK A-03', bins: makeBins('A', 3, 2) }, { name: 'RACK A-04', bins: makeBins('A', 4, 3) },
        { name: 'RACK A-05', bins: makeBins('A', 5, 3) },
      ] },
    { zone: 'B', label: 'Zone B — 냉장보관 구역', utilPct: 52, available: 13, total: 28,
      racks: [
        { name: 'RACK B-01', bins: makeBins('B', 1, 3) }, { name: 'RACK B-02', bins: makeBins('B', 2, 3) },
        { name: 'RACK B-03', bins: makeBins('B', 3, 4) }, { name: 'RACK B-04', bins: makeBins('B', 4, 3) },
        { name: 'RACK B-05', bins: makeBins('B', 5, 2) },
      ] },
    { zone: 'C', label: 'Zone C — 대형물류 구역', utilPct: 48, available: 13, total: 25,
      racks: [
        { name: 'RACK C-01', bins: makeBins('C', 1, 2) }, { name: 'RACK C-02', bins: makeBins('C', 2, 3) },
        { name: 'RACK C-03', bins: makeBins('C', 3, 3) }, { name: 'RACK C-04', bins: makeBins('C', 4, 2) },
  ...
  3: [
    { zone: 'A', label: 'Zone A — 일반보관 구역', utilPct: 85, available: 4, total: 30,
      racks: [
        { name: 'RACK A-01', bins: makeBins('A', 1, 5) }, { name: 'RACK A-02', bins: makeBins('A', 2, 5) },
        { name: 'RACK A-03', bins: makeBins('A', 3, 5) }, { name: 'RACK A-04', bins: makeBins('A', 4, 4) },
        { name: 'RACK A-05', bins: makeBins('A', 5, 5) },
      ] },
    { zone: 'B', label: 'Zone B — 냉장보관 구역', utilPct: 90, available: 2, total: 22,
      racks: [
        { name: 'RACK B-01', bins: makeBins('B', 1, 5) }, { name: 'RACK B-02', bins: makeBins('B', 2, 5) },
        { name: 'RACK B-03', bins: makeBins('B', 3, 5) }, { name: 'RACK B-04', bins: makeBins('B', 4, 4) },
        { name: 'RACK B-05', bins: makeBins('B', 5, 5) },
      ] },
    { zone: 'C', label: 'Zone C — 대형물류 구역', utilPct: 88, available: 3, total: 25,
      racks: [
        { name: 'RACK C-01', bins: makeBins('C', 1, 5) }, { name: 'RACK C-02', bins: makeBins('C', 2, 5) },
        { name: 'RACK C-03', bins: makeBins('C', 3, 5) }, { name: 'RACK C-04', bins: makeBins('C', 4, 4) },
        { name: 'RACK C-05', bins: makeBins('C', 5, 5) },
      ] },
  ],
}
